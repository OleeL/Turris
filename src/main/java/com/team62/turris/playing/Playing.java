...
public class Playing {
    ...
    public static void update(double dt) {
        if (state == PLAYING) {
            ...
            if (
                spawn_num >= wave.enemies.length - 1 &&
                !roundEnded &&
                enemies.isEmpty()
            ) {
                state = ROUND_END;
            ...
            for (int turret = 0; turret < grid.turrets.size(); turret++) {
                for (int e = 0; e < enemies.size(); e++) {
                    ...
                    Arrow arrow = grid.turrets
                        .get(turret)
                        .aimAt(enemy.getCX(), enemy.getCY());
                    if (arrow != null) {
                        arrows.add(arrow);
                        arrows_fired++;
                    }
        ...
        if (Main.window.isMousePressed(Main.window.RIGHT_MOUSE)) {
            ...
            if (tile instanceof Turret) {
                ...
                if (coins >= cost) {
                    if (turret.upgrade()) {
                        String textureName = tile
                            .getName()
                            .substring(0, tile.getName().length() - 1);
                        turret.setTexture(
                            new Texture(
    ...
    public static void draw() {
        ...
        if (selected == SELL) {
            ...
            if (grid.getEntity(x, y) instanceof Turret) Main.window.setColour(
                0f,
                1f,
                0f,
                0.3f
        ...
        // Drawing for placing turrets
            ...
            if (canPlace()) Main.window.setColour(0f, 1f, 0f, 0.3f);
            //show green
            else Main.window.setColour(1f, 0f, 0f, 0.5f); //show red
    ...
    public static boolean canPlace() {
        ...
        String tile = grid.getTile(
            (int) (temp_tile_x / grid.getTileSize()),
            (int) (temp_tile_y / grid.getTileSize())
        );
        if (
            tile.equals(Grid.BLANK) || tile.contains(Grid.DECORATION)
        ) return true;
        return false;
    }
    ...
    public static void place(int tower_num, int level) {
        ...
        if (
            Main.window.isMousePressed(Main.window.LEFT_MOUSE) &&
            !gui.isClicked() &&
            canPlace() &&
            coins >= turret_cost
        ) {
