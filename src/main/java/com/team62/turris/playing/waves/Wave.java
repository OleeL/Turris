...
import com.team62.turris.enemies.*;
...
public class Wave {
    ...
    public void produceWave(int round, String difficulty, boolean continuous) {
        try {
            ...
            File file = new File(Main.assetPath(difficulty));
            Scanner inputStream;
    ...
    private void spawnWave(
        String[] string_enemies,
        String[] string_spawn_delays
    ) {
        ...
        for (int i = 0; i < string_enemies.length; i++) {
            if (string_enemies[i].equals("1")) {
                enemies[i] = new Enemy_1(spawn_x, spawn_y, tile_size);
            }
            if (string_enemies[i].equals("2")) {
                enemies[i] = new Enemy_2(spawn_x, spawn_y, tile_size);
            }
            if (string_enemies[i].equals("3")) {
                enemies[i] = new Enemy_3(spawn_x, spawn_y, tile_size);
            }
        }
